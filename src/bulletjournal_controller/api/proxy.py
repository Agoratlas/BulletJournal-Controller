...
_WRITE_TOOLS = {"apply_graph_changes", "set_constant_value"}
...
async def proxy_mcp(project_id: str, request: Request):
    ...
    try:
        user, token = container.oauth_service.authenticate(
            request.headers.get("authorization"), project_id=project_id
        )
    except Exception:
        return JSONResponse(
    ...
    return await container.proxy_service.proxy_mcp(
        project_id=project_id, request=request, username=user.username
    )
...
async def proxy_http_root(
    ...
    request.app.state.container.authorization_service.require_project_viewer(
        bundle.user, project_id
    )
...
async def proxy_websocket(websocket: WebSocket, project_id: str, path: str):
    ...
    if bundle is None:
        ...
        return
    try:
        websocket.app.state.container.authorization_service.require_project_viewer(
            bundle.user, project_id
        )
    except Exception:
