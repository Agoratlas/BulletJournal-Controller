...
async def token(request: Request):
    ...
    if grant_type == 'authorization_code':
        ...
        result = service.refresh(str(form.get('refresh_token') or ''), client_id=client_id, resource=resource)
    else:
...
def _consent_page(*, client_name: str, project_id: str, scopes: list[str], values: dict[str, str]) -> str:
    fields = '\n'.join(
        f'<input type="hidden" name="{escape(name)}" value="{escape(value)}">' for name, value in values.items()
    )
...
<head>
  ...
  <style>
    :root {{
      color-scheme: light dark; --bg: #f4efdf; --paper: #fffaf0; --ink: #1f2929;
      --muted: #61716d; --accent: #1d7b6c; --line: rgba(31, 41, 41, .14);
    }}
    @media (prefers-color-scheme: dark) {{ :root {{
      --bg: #171714; --paper: #20211d; --ink: #efe7d8; --muted: #b7afa2;
      --accent: #63c4b2; --line: rgba(239, 231, 216, .16);
    }} }}
    * {{ box-sizing: border-box; }}
    body {{
      min-height: 100vh; margin: 0; display: grid; place-items: center; padding: 24px;
      color: var(--ink); background: radial-gradient(circle at top, rgba(29,123,108,.14), transparent 34rem), var(--bg);
      font: 16px/1.5 Inter, ui-sans-serif, system-ui, sans-serif;
    }}
    main {{
      width: min(100%, 520px); padding: 32px; border: 1px solid var(--line); border-radius: 28px;
      background: var(--paper); box-shadow: 0 18px 54px rgba(39,42,40,.12);
    }}
    .eyebrow {{
      margin: 0 0 10px; color: var(--accent); font-size: .78rem; font-weight: 750;
    ...
    button {{
      width: 100%; min-height: 44px; border: 0; border-radius: 999px; color: white;
      background: var(--accent); cursor: pointer; font: inherit; font-weight: 700;
    }}
    button:hover {{ filter: brightness(1.06); }}
    button:focus-visible {{
      outline: 3px solid color-mix(in srgb, var(--accent), transparent 55%); outline-offset: 3px;
    }}
  </style>
</head>
<body>
