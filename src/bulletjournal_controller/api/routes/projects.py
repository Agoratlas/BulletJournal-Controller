...
def _project_payload(container, project, user, metrics: dict[str, object] | None = None):
    ...
    payload['metrics'] = metrics if metrics is not None else container.metrics_service.project_metrics(project)
...
def list_projects(request: Request, _user=Depends(get_current_user)):
    ...
    return [_project_payload(container, project, _user, metrics_map.get(project.project_id)) for project in projects]
    ...
    grants = container.authorization_service.normalize_role_payload(
        {
            ...
            'editors': (
                payload.editors.model_dump() if payload.editors is not None else {'all_users': True, 'user_ids': []}
            ),
        }
    )
    python_version = payload.python_version or container.instance_config.default_python_version
...
def get_project(project_id: str, request: Request, _user=Depends(get_current_user)):
    ...
    metrics = container.metrics_service.cached_project_metrics_map([project]).get(project.project_id)
    payload = _project_payload(container, project, _user, metrics)
    payload['recent_jobs'] = [job.to_api() for job in container.jobs.list_for_project(project_id)]
    return payload


@router.get('/{project_id}/lockfile')
def download_project_lockfile(project_id: str, request: Request, _user=Depends(get_current_user)):
    container = request.app.state.container
    project = container.authorization_service.require_project_viewer(_user, project_id)
...
def download_project_export(
    ...
    archive_path = container.instance_paths.exports_dir / ExportService.download_filename(
        project_id=project.project_id,
        mode=export_mode,
    )
...
@router.get('/{project_id}/mcp-config/opencode')
def download_opencode_mcp_config(project_id: str, request: Request, _user=Depends(get_current_user)):
    container = request.app.state.container
    project = container.authorization_service.require_project_viewer(_user, project_id)
    archive, filename = container.mcp_config_service.opencode_archive(project.project_id)
    return Response(
        content=archive,
        media_type='application/zip',
        headers={
