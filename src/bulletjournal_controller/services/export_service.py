...
class ExportService:
    ...
    def import_project(
        ...
        *,
        ...
        if self.projects.get(target_project_id) is not None:
            raise ConflictError(f'Project {target_project_id} already exists in controller metadata.')
    ...
    def archive_project(
        ...
        *,
        ...
        target_path = archive_dir / self.archive_filename(project.project_id, encrypted=encrypted)
        if target_path.exists():
            raise ConflictError(f'Archive already exists for project {project.project_id}: {target_path.name}')
        ...
        with tempfile.TemporaryDirectory(dir=archive_dir) as temp_dir_name:
            ...
            log_writer(f'creating full archive for project {project.project_id} at {export_path.name}')
            self.export_project(
                project=project,
            ...
            if encrypted:
                log_writer(f'encrypting archive with openssl to {target_path.name}')
    ...
    def resolve_archive_dir(self) -> Path:
        ...
        if not candidate.is_absolute():
            raise ValidationError('BULLETJOURNAL_ARCHIVE_DIR must be an absolute path.')
        return candidate
    ...
    def _encrypt_archive(self, *, source_path: Path, target_path: Path) -> None:
        ...
        try:
            subprocess.run(  # noqa: S603 - OpenSSL invocation uses controller-owned arguments and environment.
                command,
                check=True,
                capture_output=True,
                text=True,
            ...
            raise ValidationError('OpenSSL is required to encrypt archives but was not found on PATH.') from exc
        except subprocess.CalledProcessError as exc:
            detail = (exc.stderr or exc.stdout or '').strip()
            raise ValidationError(f'Archive encryption failed{": " + detail if detail else "."}') from exc
    ...
    def _reconstruct_project_record(
        ...
        *,
        ...
        bulletjournal_version = str(self._resolve_bulletjournal_version(resolved_dependencies) or '0.1.0')
    ...
    def _archive_project_id(archive_path: Path) -> str:
        ...
        with zipfile.ZipFile(archive_path, 'r') as archive:
            project_json = json.loads(archive.read('metadata/project.json').decode('utf-8'))
    ...
    def _resolve_bulletjournal_version(dependencies: list[str]) -> str | None:
        for line in dependencies:
            if ExportService._dependency_identity(line) not in MANAGED_RUNTIME_PACKAGE_ALIASES:
                continue
            if '==' in line:
