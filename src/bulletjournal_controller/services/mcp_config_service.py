    ...
    def opencode_archive(self, project_id: str) -> tuple[bytes, str]:
        try:
            ...
            raise ConfigurationError(
                "Controller AGENTS.md is missing or unreadable. "
                "Run init-instance to seed it."
            ) from exc
        if len(agents) > MAX_AGENTS_BYTES:
            raise ConfigurationError(
                "Controller AGENTS.md exceeds the 64 KiB setup-bundle limit."
            )
        ...
        if len(result) > MAX_ARCHIVE_BYTES:
            raise ConfigurationError(
                "Generated MCP setup archive exceeds the 128 KiB limit."
            )
