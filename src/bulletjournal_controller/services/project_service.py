...
from bulletjournal_controller.domain.enums import InstallStatus, ProjectStatus, ProjectStatusReason
...
from bulletjournal_controller.domain.rules import ensure_transition_allowed, validate_project_id, validate_status_reason
from bulletjournal_controller.storage.instance_fs import InstancePaths, ProjectPaths, create_project_root, delete_project_root
from bulletjournal_controller.storage.repositories import JobRepository, ProjectRepository
...
class ProjectService:
    ...
    def set_status(self, *, project_id: str, status: str, status_reason: str | None = None, **changes) -> ProjectRecord:
    ...
    def update_limits(self, *, project_id: str, cpu_limit_millis: int, memory_limit_bytes: int, gpu_enabled: bool) -> ProjectRecord:
    ...
    def mark_installing(self, project_id: str) -> ProjectRecord:
        ...
        if project.status in {ProjectStatus.CREATING.value, ProjectStatus.ERROR.value}:
            return self.set_status(project_id=project_id, status=ProjectStatus.INSTALLING.value, status_reason=None, **updates)
        return self.projects.update(project_id, **updates)

    def mark_install_succeeded(self, project_id: str, *, lock_sha256: str) -> ProjectRecord:
        project = self.get_project(project_id)
        updates = {
        ...
        if project.status == ProjectStatus.INSTALLING.value:
            return self.set_status(project_id=project_id, status=ProjectStatus.STOPPED.value, status_reason=None, **updates)
        return self.projects.update(project_id, **updates)
    ...
    def start_project(self, project_id: str) -> ProjectRecord:
        ...
        if project.status == ProjectStatus.RUNNING.value:
            return project
        if project.status not in {ProjectStatus.STOPPED.value, ProjectStatus.ERROR.value}:
            raise ConflictError(f'Cannot start project from status {project.status}.')
        self.set_status(project_id=project_id, status=ProjectStatus.STARTING.value, status_reason=None)
        project = self.get_project(project_id)
    ...
    def stop_project(self, project_id: str, *, reason: str | None = None) -> ProjectRecord:
        ...
        self.set_status(project_id=project_id, status=ProjectStatus.STOPPING.value, status_reason=None)
        project = self.get_project(project_id)
    ...
    def apply_runtime_status(self, *, project_id: str, status_payload: dict[str, object]) -> ProjectRecord:
        ...
        last_edit_at = max([value for value in [graph_edit, notebook_edit] if isinstance(value, str)], default=None)
