...
class UserRepository(BaseRepository[UserRecord]):
    ...
    def create(self, *, user_id: str, username: str, display_name: str, password_hash: str, is_active: bool) -> UserRecord:
        now = utc_now_iso()
        with self.db.transaction() as connection:
            connection.execute(
                'INSERT INTO users (user_id, username, display_name, password_hash, is_active, created_at, updated_at, last_login_at) '
                'VALUES (?, ?, ?, ?, ?, ?, ?, NULL)',
                (user_id, username, display_name, password_hash, int(is_active), now, now),
            )
    ...
    def touch_last_login(self, user_id: str) -> None:
        now = utc_now_iso()
        with self.db.transaction() as connection:
            connection.execute('UPDATE users SET last_login_at = ?, updated_at = ? WHERE user_id = ?', (now, now, user_id))
...
class SessionRepository(BaseRepository[SessionRecord]):
        ...
        *,
        ...
        with self.db.transaction() as connection:
            connection.execute(
                ...
                'VALUES (?, ?, ?, ?, ?, NULL, ?, ?, ?)',
                (session_id, user_id, secret_hash, created_at, expires_at, created_at, user_agent, remote_addr),
            )
    ...
    def revoke(self, session_id: str) -> None:
        with self.db.transaction() as connection:
            connection.execute('UPDATE sessions SET revoked_at = ? WHERE session_id = ?', (utc_now_iso(), session_id))
...
class ProjectRepository(BaseRepository[ProjectRecord]):
    ...
    def update(self, project_id: str, **changes: Any) -> ProjectRecord:
        ...
        with self.db.transaction() as connection:
            connection.execute(f'UPDATE projects SET {assignments} WHERE project_id = ?', params)
            row = connection.execute('SELECT * FROM projects WHERE project_id = ?', (project_id,)).fetchone()
        return self._row_to_model(row)  # type: ignore[return-value]
...
class JobRepository(BaseRepository[JobRecord]):
    ...
    def create(self, **data: Any) -> JobRecord:
        ...
        placeholders = ', '.join('?' for _ in data)
        with self.db.transaction() as connection:
            connection.execute(f'INSERT INTO jobs ({columns}) VALUES ({placeholders})', tuple(data.values()))
    ...
    def update(self, job_id: str, **changes: Any) -> JobRecord:
        ...
        with self.db.transaction() as connection:
            connection.execute(f'UPDATE jobs SET {assignments} WHERE job_id = ?', params)
            row = connection.execute('SELECT * FROM jobs WHERE job_id = ?', (job_id,)).fetchone()
        return self._row_to_model(row)  # type: ignore[return-value]
