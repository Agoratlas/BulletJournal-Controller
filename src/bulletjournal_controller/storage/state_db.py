...
class StateDB:
            ...
            for name, sql in MIGRATIONS:
                ...
                try:
                    ...
                    if 'duplicate column name' not in str(exc).lower():
                        raise
                if (
                    name == '009_project_rbac'
                    and self._table_exists(connection, 'projects')
                    and self._table_exists(connection, 'users')
                ):
                    connection.execute(
                        'INSERT OR IGNORE INTO project_role_grants '
                        '(project_id, subject_kind, user_id, role, created_at, updated_at) '
                        "SELECT p.project_id, 'user', p.created_by_user_id, 'project_admin', "
    ...
    def _table_exists(connection: sqlite3.Connection, name: str) -> bool:
        return (
            connection.execute('SELECT 1 FROM sqlite_master WHERE type = ? AND name = ?', ('table', name)).fetchone()
            is not None
        )
