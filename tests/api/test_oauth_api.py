...
def test_code_exchange_accepts_omitted_resource(instance_root) -> None:
    ...
    assert tokens['token_type'] == 'Bearer'  # noqa: S105 - OAuth protocol token assertion.
...
def test_token_endpoint_returns_tokens_without_resource(instance_root) -> None:
    ...
    user = container.auth_service.create_user(username='analyst', display_name='Analyst', password='secret-pass')
    ...
    client_id = service.register_client({'redirect_uris': ['http://127.0.0.1:19876/mcp/oauth/callback']})['client_id']
    verifier = 'verifier'
    challenge = base64.urlsafe_b64encode(hashlib.sha256(verifier.encode()).digest()).rstrip(b'=').decode()
    redirect = service.authorize(
        user=user,
    ...
    assert response.status_code == 200
    assert response.json()['token_type'] == 'Bearer'  # noqa: S105 - OAuth protocol token assertion.
