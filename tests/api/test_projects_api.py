...
def test_project_list_does_not_require_journal_mode_reset(instance_root, server_config, monkeypatch) -> None:
...
def test_project_create_rejects_gpu_when_instance_gpu_disabled(instance_root, server_config) -> None:
    app = create_app(instance_root=instance_root, server_config=server_config)
    container: ServiceContainer = app.state.container
    container.auth_service.create_user(username='admin', display_name='Admin', password='secret-pass')
    with _auth_client(app) as client:
...
def test_project_lockfile_download_returns_404_when_missing(instance_root, server_config) -> None:
    app = create_app(instance_root=instance_root, server_config=server_config)
    container: ServiceContainer = app.state.container
    user = container.auth_service.create_user(username='admin', display_name='Admin', password='secret-pass')
    project = container.project_service.create_project(
...
def test_project_export_download_supports_modes(instance_root, server_config) -> None:
    ...
    (project_root / 'checkpoints' / 'cp-1' / 'notebooks' / 'snap.py').parent.mkdir(parents=True, exist_ok=True)
    (project_root / 'checkpoints' / 'cp-1' / 'notebooks' / 'snap.py').write_text("print('cp')\n", encoding='utf-8')
    ...
    assert (
        'attachment; filename="bulletjournal_export_study-a_code_and_data.zip"'
        in response.headers['content-disposition']
    )
    archive_path = container.instance_paths.exports_dir / 'bulletjournal_export_study-a_code_and_data.zip'
    ...
    assert not any(name.startswith('checkpoints/') and name != 'checkpoints/' for name in names)
...
def test_project_create_requires_bulletjournal_dependency_line(instance_root, server_config) -> None:
    app = create_app(instance_root=instance_root, server_config=server_config)
    container: ServiceContainer = app.state.container
    container.auth_service.create_user(username='admin', display_name='Admin', password='secret-pass')
    with _auth_client(app) as client:
