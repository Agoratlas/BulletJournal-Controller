...
def _login(app, username: str) -> TestClient:
    ...
    response = client.post('/api/v1/session/login', json={'username': username, 'password': 'secret-pass'})
...
def test_project_roles_filter_visibility_and_enforce_admin_actions(instance_root, server_config) -> None:
    app = create_app(instance_root=instance_root, server_config=server_config)
    container: ServiceContainer = app.state.container
    owner = container.auth_service.create_user(username='owner', display_name='Owner', password='secret-pass')
    editor = container.auth_service.create_user(username='editor', display_name='Editor', password='secret-pass')
    container.auth_service.create_user(username='stranger', display_name='Stranger', password='secret-pass')
...
def test_all_users_is_persisted_and_server_admin_bypasses_grants(instance_root, server_config) -> None:
    app = create_app(instance_root=instance_root, server_config=server_config)
    container: ServiceContainer = app.state.container
    owner = container.auth_service.create_user(username='owner', display_name='Owner', password='secret-pass')
    ...
    future_user = container.auth_service.create_user(username='future', display_name='Future', password='secret-pass')
