...
def test_create_user_with_invalid_password_hash_raises_validation_error(instance_root, server_config) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
...
def test_create_user_with_existing_username_raises_conflict(instance_root, server_config) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
...
def test_create_or_update_user_with_password_hash_updates_existing_user(instance_root, server_config) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
...
def test_create_or_update_user_with_password_hash_revokes_existing_sessions(instance_root, server_config) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
    ...
    user = service.create_user(username='admin', display_name='Admin', password='secret-pass')
    bundle = service.create_session(user=user, user_agent='pytest', remote_addr='127.0.0.1')
...
def test_create_or_update_user_with_same_password_hash_keeps_existing_sessions(instance_root, server_config) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
...
def test_resolve_session_skips_touch_for_recent_activity(instance_root, server_config, monkeypatch) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
    ...
    def spy_touch(session_id: str, *, expires_at: str, only_if_last_seen_at: str | None = None) -> None:
...
def test_resolve_session_refreshes_stale_activity(instance_root, server_config, monkeypatch) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
...
def test_resolve_session_ignores_locked_touch_for_valid_session(instance_root, server_config, monkeypatch) -> None:
    db = StateDB(instance_root / 'metadata' / 'state.db')
    service = AuthService(
        users=UserRepository(db),
        sessions=SessionRepository(db),
    ...
    def locked_touch(session_id: str, *, expires_at: str, only_if_last_seen_at: str | None = None) -> None:
