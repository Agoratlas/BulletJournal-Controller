...
def test_create_user_accepts_password_hash_from_stdin(instance_root, monkeypatch: pytest.MonkeyPatch) -> None:
    password_hash = import_module('argon2').PasswordHasher().hash('secret-pass')
...
def test_cli_parser_accepts_delete_user() -> None:
    args = build_parser().parse_args(['delete-user', './instance', '--username', 'admin'])
