...
def test_delete_user_rejects_a_sole_project_admin(instance_root) -> None:
    ...
    with pytest.raises(ConflictError, match="sole admin.*research"):
...
def test_delete_user_removes_role_grants_sessions_and_reassigns_history(instance_root) -> None:
    ...
    session = container.auth_service.create_session(
        user=admin, user_agent="pytest", remote_addr="127.0.0.1"
    )
    now = utc_now_iso()
    ...
    assert [grant.user_id for grant in container.role_grants.list_for_project("research")] == [
        replacement.user_id
    ]
