...
def test_project_status_is_one_hot_and_deleted_project_series_are_removed() -> None:
    ...
    observability.observe_project_request(project_id='study-a', status_class='2xx', duration=0.1, app_duration=0.05)
...
def test_project_configured_limits_are_exported_without_runtime_metrics() -> None:
    ...
    assert 'bulletjournal_controller_project_memory_limit_bytes{project_id="stopped-project"} 8.589934592e+09' in output
    assert (
        'bulletjournal_controller_project_disk_soft_limit_bytes{project_id="stopped-project"} 1.073741824e+11' in output
    )
...
def test_controller_route_uses_template_or_bounded_fallback() -> None:
    assert (
        controller_route({'route': type('Route', (), {'path': '/api/v1/projects/{project_id}'})()})
