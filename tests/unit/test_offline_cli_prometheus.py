...
def test_cleanup_instance_does_not_require_prometheus_api_key(
    ...
    monkeypatch.setattr(RuntimeService, 'cleanup_instance_containers', lambda self: ['runtime'])
...
def test_export_project_does_not_require_prometheus_api_key(
    ...
    result = export_project(str(authenticated_metrics_instance), 'study-a', str(tmp_path / 'study-a.zip'))
