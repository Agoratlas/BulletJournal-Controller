...
class DummyRuntimeConfigService:
    runtime_config = type('RuntimeConfig', (), {'runtime_image_name': 'bulletjournal-runtime:local'})()
...
def test_create_project_rejects_gpu_when_instance_gpu_disabled(tmp_path) -> None:
    ...
    with pytest.raises(ValidationError, match=r'GPU is not supported on this instance\.'):
        service.create_project(
            project_id='study-a',
            created_by_user_id=user.user_id,
            python_version='3.11',
