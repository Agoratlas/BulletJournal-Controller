...
def test_runtime_config_additional_mounts_defaults_read_only_to_false(
        ...
        json.dumps(
            {
                ...
                'default_dependencies_file': 'default-dependencies.txt',
                'env_file': '.env',
                'additional_mounts': [{'source': str(mount_source), 'target': '/opt/shared-data'}],
            }
        ),
        encoding='utf-8',
    )
    ...
    assert service.runtime_config.additional_mounts == ((mount_source.resolve(), '/opt/shared-data', False),)
...
def test_runtime_config_rejects_invalid_additional_mounts(tmp_path: Path, mounts: object, message: str) -> None:
    instance_paths = init_instance_root(tmp_path / 'instance')
    instance_paths.local_runtime_json_path.write_text(
        json.dumps(
            {
